...
extern volatile uint8_t    time_elapsed_flag;
extern volatile uint8_t    pkt_timeout;
extern volatile pwm_data_t pwm_data;
extern volatile uint8_t    ppm_index;
...
void TIM3_IRQHandler(void)
{
  ...
  if (LL_TIM_IsActiveFlag_CC1(TIM3)) {
    ...
    uint16_t duration = (uint16_t)(capture - last_capture);
    last_capture = capture;

    if (duration > 2100) {
      ppm_index = 0;
      data_ready_flag = 1;
    } else if (ppm_index < CH_NUM)
      pwm_data.ch[ppm_index++] = duration;
  }
#endif
